...
function renderMissedWordsList() {
    ...
    missedWords.forEach(word => {
        missedWordItem = document.createElement("li");
        missedWordItem.innerText = word.english; // Ändra från 'swedish' till 'english'
...
function loadQuestion() {
    ...
    document.getElementById("word-text").innerText = currentWord.english; // Visa det engelska ordet nu

    let shuffledChoices = [...currentWord.swedish]; // Slumpa de svenska alternativen nu
    shuffledChoices = shuffleArray(shuffledChoices);

    const correctAnswerText = currentWord.swedish[currentWord.correct];
...
// Speak the English word (updated for the reverse scenario)
function speakEnglish() {
    const wordList = isRetry ? previousRoundMissedWords : vocabulary;
    if (currentQuestion >= wordList.length) {
        return;
    }
    const englishWord = wordList[currentQuestion].english; // Talar det engelska ordet nu
...
function checkAnswer(choice) {
    ...
    if (isCorrect) {
        ...
        if (isRetry) {
            missedWords = missedWords.filter(word => word.english !== currentWord.english); // Kolla på 'english' nu
        ...
        if (!missedWords.some(word => word.english === currentWord.english)) { // Lägger till 'english' istället för 'swedish'
...
function showModal() {
    ...
    if (missedWords.length === 1) {
        modalMessage.innerHTML = `You got "${missedWords[0].english}" wrong. Do you want to try it again?`; // Ändrat till 'english'
